...
public class GameOfLifePanel extends JPanel {
    ...
    public GameOfLifePanel(GameOfLife game) {
        this.game = game;
        Hexagon hexagon = getHexagon(BOARD_WIDTH - 1, BOARD_HEIGHT -1, new Coordinates(BOARD_WIDTH - 1, BOARD_HEIGHT -1));
    ...
    private void paintHex(int x, int y, Graphics g, boolean fill) {
        Hexagon hexagon = getHexagon(x, y, new Coordinates(x,y));
        if (fill) {
    ...
    private Hexagon getHexagon(int x, int y, Coordinates centerCoordinates) {
        ...
        return new Hexagon(radius, gridCoordinates);
    }
}
